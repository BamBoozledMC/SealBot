...
mc.on("message", (chatMsg) => {
    ...
    if (msg.startsWith("Guild >") && msg.includes(":")) {
        ...
        //     mc.chat("/playtime")
        // }
        if (sentMsgNoSpaces.toLowerCase().startsWith("!ll")) {
            db.get("msg.lurklist")
            mc.chat("/gc People Lurking: " + db.get("msg.lurklist"))
    ...
    if (msg.includes("left.")) {
        ...
        //channel.send(embed);
        client.guilds.get(config["discord-guild"]).channels.get(config["discord-channel"]).send(embed);
        db.set("msg.lurklist", config.get("msg.lurklist").replace(`, ${sender}`, ""))
    }
...
if(message.content.toLowerCase().startsWith(prefix + "setrank")) {
    let args = message.content.split(" ");
    let rankset = args.slice(1).join(" ")
